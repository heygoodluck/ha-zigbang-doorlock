...
*   **카드키/지문 관리**: Home Assistant 서비스와 선택 엔티티로 등록된 카드키와 지문 목록을 조회하고 삭제할 수 있습니다.
...
통합 구성요소가 카드키/지문 목록을 자동 조회해 아래 `select` 엔티티를 생성합니다.

* `카드키 선택`
* `지문 선택`

대시보드나 엔티티 상세 화면에서 삭제할 카드키/지문을 드롭다운으로 고른 뒤, Home Assistant의 **개발자 도구 → 서비스**에서 아래 서비스를 호출하면 됩니다.

#### 선택한 카드키 삭제
```yaml
service: zigbang_doorlock.delete_selected_card_key
data:
  entity_id: select.card_key_selector
```

#### 선택한 지문 삭제
```yaml
service: zigbang_doorlock.delete_selected_fingerprint
data:
  entity_id: select.fingerprint_selector
```

등록 목록만 JSON으로 확인하고 싶으면 아래 조회 서비스를 사용할 수 있습니다.
